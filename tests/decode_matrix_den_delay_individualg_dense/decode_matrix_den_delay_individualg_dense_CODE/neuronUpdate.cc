...
__kernel void updateNeuronsKernel(__global scalar* d_denDelaySyn, __global unsigned int* d_glbSpkCntPre, __global unsigned int* d_glbSpkPre, __global scalar* d_inSynSyn, __global scalar* d_xPost, volatile unsigned int denDelayPtrSyn, float t) {
    const size_t localId = get_local_id(0);
    const unsigned int id = get_global_id(0);
...
void updateNeuronsProgramKernels() {
    ...
    CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(0, d_denDelaySyn));
...
void updateNeurons(float t) {
     ...
     {
        CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(5, denDelayPtrSyn));
        CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(6, t));
