...
float* denDelaySyn;
cl::Buffer d_denDelaySyn;
unsigned int denDelayPtrSyn;
cl::Buffer d_denDelayPtrSyn;
...
void pushPostSpikesToDevice(bool uninitialisedOnly) {
    ...
    if(!uninitialisedOnly) {
        CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_glbSpkPost, CL_TRUE, 0, 1 * sizeof(unsigned int), glbSpkPost));
    }
}
...
void pushxPostToDevice(bool uninitialisedOnly) {
    if(!uninitialisedOnly) {
        CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_xPost, CL_TRUE, 0, 1 * sizeof(scalar), xPost));
    }
}
...
void pushgSynToDevice(bool uninitialisedOnly) {
    if(!uninitialisedOnly) {
        CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_gSyn, CL_TRUE, 0, 10 * sizeof(scalar), gSyn));
    }
}

void pushdSynToDevice(bool uninitialisedOnly) {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_dSyn, CL_TRUE, 0, 10 * sizeof(uint8_t), dSyn));
}

void pushinSynSynToDevice(bool uninitialisedOnly) {
    if(!uninitialisedOnly) {
        CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_inSynSyn, CL_TRUE, 0, 1 * sizeof(float), inSynSyn));
    }
}

void pushSynStateToDevice(bool uninitialisedOnly) {
    pushgSynToDevice(uninitialisedOnly);
    pushdSynToDevice(uninitialisedOnly);
...
void pullPostSpikesFromDevice() {
    ...
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_glbSpkPost, CL_TRUE, 0, 1 * sizeof(unsigned int), glbSpkPost));
}
...
void pullxPostFromDevice() {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_xPost, CL_TRUE, 0, 1 * sizeof(scalar), xPost));
}
...
void pullgSynFromDevice() {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_gSyn, CL_TRUE, 0, 10 * sizeof(scalar), gSyn));
}

void pulldSynFromDevice() {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_dSyn, CL_TRUE, 0, 10 * sizeof(uint8_t), dSyn));
}

void pullinSynSynFromDevice() {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_inSynSyn, CL_TRUE, 0, 1 * sizeof(float), inSynSyn));
}
...
void allocateMem() {
    ...
    glbSpkPost = (unsigned int*)calloc(1, sizeof(unsigned int));
    d_glbSpkPost = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 1 * sizeof(unsigned int), glbSpkPost);
    xPost = (scalar*)calloc(1, sizeof(scalar));
    d_xPost = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 1 * sizeof(scalar), xPost);
    ...
    // postsynaptic variables
    // ------------------------------------------------------------------------
    inSynSyn = (float*)calloc(1, sizeof(float));
    d_inSynSyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 1 * sizeof(float), inSynSyn);
    denDelaySyn = (float*)calloc(10, sizeof(float));
    d_denDelaySyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 10 * sizeof(float), denDelaySyn);
    ...
    // synapse variables
    // ------------------------------------------------------------------------
    gSyn = (scalar*)calloc(10, sizeof(scalar));
    d_gSyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 10 * sizeof(scalar), gSyn);
    dSyn = (uint8_t*)calloc(10, sizeof(uint8_t));
    d_dSyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 10 * sizeof(uint8_t), dSyn);
