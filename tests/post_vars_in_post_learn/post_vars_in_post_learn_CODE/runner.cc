...
void pushxpreToDevice(bool uninitialisedOnly) {
    if(!uninitialisedOnly) {
        CHECK_OPENCL_ERRORS(commandQueue.enqueueWriteBuffer(d_xpre, CL_TRUE, 0, 10 * sizeof(scalar), xpre));
    }
}
...
void pullxpreFromDevice() {
    CHECK_OPENCL_ERRORS(commandQueue.enqueueReadBuffer(d_xpre, CL_TRUE, 0, 10 * sizeof(scalar), xpre));
}
