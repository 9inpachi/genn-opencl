...
__kernel void updatePostsynapticKernel(__global unsigned int* d_glbSpkCntpost, __global unsigned int* d_glbSpkpost, __global scalar* d_wsyn0, __global scalar* d_wsyn1, __global scalar* d_wsyn2, __global scalar* d_wsyn3, __global scalar* d_wsyn4, __global scalar* d_wsyn5, __global scalar* d_wsyn6, __global scalar* d_wsyn7, __global scalar* d_wsyn8, __global scalar* d_wsyn9, __global scalar* d_xpost, volatile unsigned int spkQuePtrpre, float t) {
    const size_t localId = get_local_id(0);
    const unsigned int id = get_global_id(0);
    __local unsigned int shSpk[32];
    // syn0
    if(id < 32) {
        const unsigned int preReadDelayOffset = spkQuePtrpre * 10;
    ...
    if(id >= 32 && id < 64) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 9) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 64 && id < 96) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 8) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 96 && id < 128) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 7) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 128 && id < 160) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 6) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 160 && id < 192) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 5) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 192 && id < 224) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 4) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 224 && id < 256) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 3) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 256 && id < 288) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 2) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
    ...
    if(id >= 288 && id < 320) {
        ...
        const unsigned int preReadDelayOffset = ((spkQuePtrpre + 1) % 10) * 10;
        const unsigned int numSpikes = d_glbSpkCntpost[0];
        const unsigned int numSpikeBlocks = (numSpikes + 31) / 32;
        for (unsigned int r = 0; r < numSpikeBlocks; r++) {
            const unsigned int numSpikesInBlock = (r == numSpikeBlocks - 1) ? ((numSpikes - 1) % 32) + 1 : 32;
...
void updateSynapsesProgramKernels() {
    ...
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(2, d_wsyn0));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(3, d_wsyn1));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(4, d_wsyn2));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(5, d_wsyn3));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(6, d_wsyn4));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(7, d_wsyn5));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(8, d_wsyn6));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(9, d_wsyn7));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(10, d_wsyn8));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(11, d_wsyn9));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(12, d_xpost));
    CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(13, spkQuePtrpre));
}

void updateSynapses(float t) {
     {
        CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(13, spkQuePtrpre));
        CHECK_OPENCL_ERRORS(updatePostsynapticKernel.setArg(14, t));
