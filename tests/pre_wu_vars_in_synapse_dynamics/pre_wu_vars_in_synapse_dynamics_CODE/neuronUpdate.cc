...
__kernel void preNeuronResetKernel(__global unsigned int* d_glbSpkCntpost, __global unsigned int* d_glbSpkCntpre, volatile unsigned int spkQuePtrpre) {
    ...
    if(id == 0) {
        ...
        spkQuePtrpre = (spkQuePtrpre + 1) % 21;
        d_glbSpkCntpre[0] = 0;
    }
}

__kernel void updateNeuronsKernel(const float DT, __global unsigned int* d_glbSpkCntpost, __global unsigned int* d_glbSpkCntpre, __global unsigned int* d_glbSpkpost, __global unsigned int* d_glbSpkpre, __global scalar* d_inSynsyn, __global scalar* d_ssyn, volatile unsigned int spkQuePtrpre, float t) {
    const size_t localId = get_local_id(0);
    const unsigned int id = get_global_id(0);
    ...
    if(id >= 32 && id < 64) {
        ...
        const unsigned int readDelayOffset = (((spkQuePtrpre + 20) % 21) * 10);
        ...
        if(lid < 10) {
            ...
            // test for and register a true spike
            if (t >= (scalar)lid && fmod(t - (scalar)lid, 10.0f)< 1e-4f) {
                const unsigned int spkIdx = atomic_add(&shSpkCount, 1);
                shSpk[spkIdx] = lid;
            }
            else {
                d_ssyn[writeDelayOffset + lid] = d_ssyn[readDelayOffset + lid];
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
        if (localId == 0) {
        ...
        if (localId < shSpkCount) {
            const unsigned int n = shSpk[localId];
             {
                // perform presynaptic update required for syn
