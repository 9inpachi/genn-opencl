...
uint32_t* indsyn;
cl::Buffer d_indsyn;
unsigned int* synRemapsyn;
cl::Buffer d_synRemapsyn;
...
void allocateMem() {
    ...
    // postsynaptic variables
    // ------------------------------------------------------------------------
    inSynsyn = (float*)malloc(10 * sizeof(float));
    ...
    indsyn = (uint32_t*)malloc(10 * sizeof(uint32_t));
    d_indsyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 10 * sizeof (uint32_t), indsyn);
    synRemapsyn = (unsigned int*)malloc(11 * sizeof(unsigned int));
    d_synRemapsyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 11 * sizeof (unsigned int), synRemapsyn);
    
    // ------------------------------------------------------------------------
    // synapse variables
    // ------------------------------------------------------------------------
    wsyn = (scalar*)malloc(10 * sizeof(scalar));
    d_wsyn = cl::Buffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 10 * sizeof (scalar), wsyn);
    ssyn = (scalar*)malloc(210 * sizeof(scalar));
