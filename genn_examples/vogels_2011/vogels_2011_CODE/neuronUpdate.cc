...
__kernel void updateNeuronsKernel(const float DT, __global scalar* d_RefracTimeE, __global scalar* d_RefracTimeI, __global scalar* d_VE, __global scalar* d_VI, __global unsigned int* d_glbSpkCntE, __global unsigned int* d_glbSpkCntI, __global unsigned int* d_glbSpkE, __global unsigned int* d_glbSpkI, __global scalar* d_inSynEE, __global scalar* d_inSynEI, __global scalar* d_inSynIE, __global scalar* d_inSynII, __global unsigned int* d_sTE, __global unsigned int* d_sTI, float t) {
    const size_t localId = get_local_id(0);
    const unsigned int id = get_global_id(0);
    ...
    if(id < 2016) {
        ...
        if(id < 2000) {
            ...
            if (lRefracTime <= 0.0f) {
              scalar alpha = ((Isyn + (2.00000000000000011e-01f)) * (1.00000000000000000e+02f)) + (-6.00000000000000000e+01f);
...
void updateNeuronsProgramKernels() {
    ...
    CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(13, d_sTE));
    CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(14, d_sTI));
}
...
void updateNeurons(float t) {
     ...
     {
        CHECK_OPENCL_ERRORS(updateNeuronsKernel.setArg(15, t));
        
        const cl::NDRange globalWorkSize(2528, 1);
        const cl::NDRange localWorkSize(32, 1);
